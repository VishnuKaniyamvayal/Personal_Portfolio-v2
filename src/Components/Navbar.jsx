  ...
  return (
    <div className={`bg-transparent flex justify-between pt-5 pl-5 pr-2 ease-in duration-300 max-w-[1400px] ${dark?"bg-black":"bg-white"}}`} >
        ...
        <ul className='md:flex my-auto text-white hidden bg-transparent ${dark?"bg-[#00300]":"bg-white"}'>
            ...
            <button className='mr-[-30px] ' onClick={dark?themeChangeToLight:themeChangeToDark}><li className={`bg-transparent p-4 cursor-pointer pl-6 ${dark?"text-white hover:text-[#6FEDD6]":"text-gray-500 hover:text-black"} `}>{dark?<MdLightMode size={23}/>:<BsFillMoonFill size={23} />}</li></button>
            
        </ul>
        <div  className='text-white md:hidden  flex'>
        <button className=' ml-6' onClick={dark?themeChangeToLight:themeChangeToDark}><li className={`mr-12 list-none bg-transparent cursor-pointer ${dark?"text-white hover:text-[#6FEDD6]":"text-gray-500 hover:text-black"} `}>{dark?<MdLightMode size={23}/>:<BsFillMoonFill size={23} />}</li></button>
